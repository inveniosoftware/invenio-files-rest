...
class StorageFactory:
    ...
    You may subclass this factory to implement custom behaviour. If you do this, remember to set
    FILES_REST_STORAGE_FACTORY to the right import path for your subclass.
    """
    ...
    def __call__(
        ...
        """Return a FileStorage instance for a file, for manipulating file contents.

        This requires that the fileinstance already has an associated storage backend. If not,
        you should call initialize() instead to initialize storage for the file instance.
        """
    ...
    def initialize(
        ...
        If provided, `preferred_location` will inform where the file will be stored, but may be ignored
        if this factory is subclassed and `get_location()` is overridden.
        """
    ...
    def get_location(self, fileinstance: FileInstance, preferred_location: Location = None) -> Location:
        ...
        This base implementation returns the preferred_location if it's provided, or the default location
        recorded in the database. This method can be overridden if some other methodology is required.
        """
        return preferred_location or Location.get_default()

    def resolve_storage_backend(self, backend_name: str) -> Type[StorageBackend]:
        """Resolve a storage backend name to the associated storage backend class.

        This base implementation resolves backends from the FILES_REST_STORAGE_BACKENDS app config
        setting.
        """
    ...
    def get_storage_backend_kwargs(
        ...
        This returns an empty dict by defaut, but can be overridden to provide backend-specific
        instantiation parameters if necessary.
        """
        return {}
    ...
    def get_suggested_uri(
        ...
        This can be overridden if your implementation requires some other file layout for storage.
        Note that individual storage backends may choose to ignore the suggested URI if they
        organise files by some other scheme.
        """
        return make_path(
            location,
            str(fileinstance.id),
