...
def as_object_version(value):
    ...
    return (
        ...
        else db.session.query(ObjectVersion).filter_by(version_id=value).one_or_none()
    )
...
class Bucket(db.Model, db.Timestamp):
    ...
    def remove(self):
        ...
        """
        with db.session.begin_nested():
            db.session.query(ObjectVersion).filter_by(bucket_id=self.id).delete(
...
class FileInstance(db.Model, db.Timestamp):
    ...
    last_check_at = db.Column(db.UTCDateTime, nullable=True)
    ...
    def validate_checksum(self, key, checksum):
        ...
        if checksum and (
            checksum.count(":") != 1
            or not all(part.strip() for part in checksum.split(":", 1))
        ):
            raise ValueError(
...
class ObjectVersion(db.Model, db.Timestamp):
    ...
    def relink_all(cls, old_file, new_file):
        ...
        with db.session.begin_nested():
            db.session.query(ObjectVersion).filter_by(file_id=str(old_file.id)).update(
